...
elseif (size(JD,2)==4),
   JD = julday(JD);
elseif (size(JD,2)==6),
   FracDay = convertdms(JD(:,4:6),'H','f');
   JD      = julday([JD(:,1:3), FracDay]);
else
...
switch Observer
 ...
 otherwise
    % rest of planets
    [X, Y, Z]   = ple_planet(JD,Observer,'XYZ');
    ObserverCoo = [X, Y, Z];
end
...
switch Planet
 ...
 otherwise
    % rest of planets
    [X, Y, Z]   = ple_planet(JD,Planet,'XYZ');
    PlanetCoo   = [X, Y, Z];
end
...
Mag = planets_magnitude(Planet, Dist(:,2), Dist(:,1), Dist(:,3),'V',0,0);
K   = 0.5.*(1 + cos(Ang(:,2)));
Mag = [Mag, K];
